...
import {
  useLang, useServices, useBudgetTiers, useTerms,
...
function resizeResponsive3D(
  ...
  // PENTING: ukur dari parent (wrapper div), BUKAN dari canvas.
  // renderer.setSize() di bawah menetapkan canvas.style.width/height
  // secara inline (perilaku standar semua renderer WebGL, termasuk ogl,
  // untuk keperluan devicePixelRatio scaling) — inline style ini
  // mengalahkan className CSS apa pun. Kalau kita baca ulang dari
  // canvas.clientWidth/Height pada resize berikutnya, yang terbaca
  // adalah ukuran yang KITA SENDIRI set sebelumnya, bukan ukuran section
  // yang sesungguhnya. Kalau pengukuran pertama kebetulan kecil (race
  // saat mount), canvas jadi terkunci kecil selamanya: baca kecil -> set
  // kecil -> baca kecil lagi, tak pernah pulih walau ResizeObserver aktif.
  // Parent wrapper tidak pernah disentuh ogl sama sekali, jadi ukurannya
  // selalu murni hasil CSS/layout — sumber pengukuran yang stabil.
  const parent = canvas.parentElement;
  const width = Math.max(parent?.clientWidth ?? canvas.clientWidth, 1);
  const height = Math.max(parent?.clientHeight ?? canvas.clientHeight, 1);
...
function Real3DScene() {
  ...
  useEffect(() => {
    ...
    // resize window), jadi pengukuran pertama bisa dapat ukuran yang
    // belum final -> objek 3D jadi kecil/kepotong. ResizeObserver
    // memantau perubahan ukuran elemen secara langsung, apapun penyebabnya.
    // PENTING: observe canvas.parentElement (wrapper), BUKAN canvas.
    // ogl men-set canvas.style.width/height sendiri lewat setSize(), jadi
    // meng-observe canvas berarti kita (tidak sengaja) mengamati perubahan
    // yang kita timbulkan sendiri, bukan perubahan layout dari section
    // induk. Wrapper tidak pernah disentuh ogl, jadi observernya jujur.
